...
module.exports = (bot, channel) =>
{
  if (!channel.guild.me.permissions.has('SEND_MESSAGES', 'VIEW_AUDIT_LOG','EMBED_LINKS', true)) {return;}
    ...
    let cCreatorU = channel.guild.fetchAuditLogs({type: 'CHANNEL_CREATE', limit: 1}).then(aLog => aLog.entries.first()).then(aLog2 => aLog2.executor).then(u => u.id);
    let cCreateE = new Discord.RichEmbed()
    .setDescription(`**${cCreatorU} created channel/category with name ${channel.name}**`)
    .setColor('GREEN')
    .setTimestamp()

    if(!logChannel) {return channel.guild.owner.send(cCreateE).catch(console.error);}
